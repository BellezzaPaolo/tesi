...
N_fine = df_pf_dtxNf_2['N_fine'].unique() #[2,4,5,6]
print(f"Unique N_fine values: {N_fine}")
N_coarse = df_pf_dtxNf['N_coarse'].unique()#[10,20]
        ...
        for nf in N_fine:
            for nc in N_coarse:
                ...
                ax.plot(subset_dtxNf['tau_fine'].values, it,
                    marker='s', linewidth=3, markersize=10, linestyle='--', color = '#17becf',
                    label=label)
                ...
                ax.plot(subset_dt['tau_fine'].values, it,
                    marker='^', linewidth=3, markersize=10,  linestyle='--', color = '#9467bd',
                    label=label)
                ...
                ax.plot(subset_dt_Nf['tau_fine'].values, it,
                    marker='<', linewidth=3, markersize=10, linestyle='--', color = '#e377c2',
                    label=label)
                ...
                pf_iterations_all = []
                for pf_subset in [subset_dtxNf, subset_dt, subset_dtxNf_2, subset_dtxNf_2x3, subset_dt_Nf]:
                ...
                ax.text(0.012, 0.135, textstr, transform=ax.transAxes, fontsize=30,
                ...
                if name == 'az' and test_case_num == '1':
                    ax.set_ylim([20, 3300])
                # Fix y-axis for az method in test case 1
                if name == 'az' and test_case_num == '2':
                    ax.set_ylim([15, 2100])
                # Fix y-axis for az method in test case 1
                if name == 'az' and test_case_num == '3':
                    ax.set_ylim([20, 4100])


                # Fix y-axis for az method in test case 1
                if name == 'L2_P' and test_case_num == '1':
                    ax.set_ylim([20, 500])
                # Fix y-axis for az method in test case 1
                if name == 'L2_P' and test_case_num == '2':
                    ax.set_ylim([20, 300])
                # Fix y-axis for az method in test case 1
                if name == 'L2_P' and test_case_num == '3':
                    ax.set_ylim([30, 1300])
                ...
                # plt.show()
                # exit()
                plt.savefig(case_folder / f'plot_{test_case_num}_alpha_{name}_Nf{nf}_Nc{nc}.png', dpi=300)
                plt.close()
