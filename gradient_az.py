...
def compute_riesz(R_u, u, w, v, u_old, beta, bcs, param = None):
    beta_c = fd.Constant(beta)

    a = 0.5 * fd.dot(fd.grad(u), fd.grad(w)) * fd.dx\
        + v * u * w * fd.dx \
        + beta_c * abs(u_old)**2 * u * w * fd.dx
    rhs = u_old * w * fd.dx
...
def assemble_forms(u, w, tau, u_old, R_u):
    ...
    for beta in beta_v:
        for tau in tau_v:
            ...
            for i in range(MaxIter):
                ...
                compute_riesz(R_u, u, w, v, u_old, beta, bcs)
                
                a, rhs = assemble_forms(u, w, tau, u_old, R_u)
